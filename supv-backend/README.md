...
# Run the application
dotnet run --project src/supv_backend.Web

# Or run with Aspire (if using)
dotnet run --project src/supv_backend.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/supv_backend.Web -s src/supv_backend.Web
dotnet run --project src/supv_backend.Web
```
